...
new class extends Component
{
    ...
    public function getDashboardRoute()
    {
        ...
        if ($user->hasRole('admin')) {
            ...
            return route('sekretariat');
        } elseif ($user->hasRole('kearsipan')) {
            return route('kearsipan');
        } elseif ($user->hasRole('layanan')) {
            return route('layanan');
        } elseif ($user->hasRole('pengembangan')) {
            return route('pengembangan');
        }
...
{{--                         
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        <div class="pt-2 pb-3 space-y-1">
            {{-- <x-responsive-nav-link :href="$this->getDashboardRoute()" :active="request()->routeIs(Request::segment(1))" wire:navigate>
